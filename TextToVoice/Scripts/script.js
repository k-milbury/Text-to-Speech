let speech = new SpeechSynthesisUtterance() // creates a new instance of the SpeechSynthesisUtterance object, which represents the text that will be spoken.

let voices = []; // initializes an empty array called voices where the available voices will be stored.
...
window.speechSynthesis.onvoiceschanged = () =>{   // is an event that fires when the list of available voices has been updated. The assigned arrow function is a callback that executes when this event occurs.
    voices = window.speechSynthesis.getVoices(); // retrieves the available voices using the getVoices() method and assigns them to the voices array.
    speech.voice = voices[0];  // sets the voice property of the SpeechSynthesisUtterance object (speech) to the first voice in the voices array
